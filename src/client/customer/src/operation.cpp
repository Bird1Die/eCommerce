...
vector<Order> GetOrderList(Context ctx){
    ...
    "SELECT i.id i.product i.price o.quantity s.shipping_status FROM orders o JOIN shipping s on s.id_order = o.id JOIN insertion i on i.id=o.product WHERE s.shipping_status='delivering' AND o.customer = %d AND o.assigned='true'", ctx.GetId());
    ...
    snprintf(buf, sizeof(buf), 
    "SELECT i.id i.product i.price o.quantity s.shipping_status FROM orders o JOIN shipping s on s.id_order = o.id JOIN insertion i on i.id=o.product WHERE s.shipping_status='pendant' AND o.customer = %d AND o.assigned='true'", ctx.GetId());
    PGresult *result = ctx.GetDB().ExecSQLtuples(buf);
    for(int i = 0; i < PQntuples(result); i++){
        int id = stoi(PQgetvalue(result, i, 0));
        string name = PQgetvalue(result, i, 1);
    ...
    for(int i = 0; i < PQntuples(result); i++){
        ...
        float price = stof(PQgetvalue(result, i, 2));
        int quantity = stoi(PQgetvalue(result, i, 3));
        s.push_back(Order(id, name, price, quantity, "assigning trasporter"));
    }
